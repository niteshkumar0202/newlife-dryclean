      ...
      <main>
        <section className="bg-gradient-to-b from-[#E8FBF5] via-[#f8fbff] to-white px-6 py-24 md:px-12 md:py-32">
          <div className="mx-auto max-w-6xl">
            <Link href="/locations" className="inline-flex items-center gap-2 text-sm font-semibold text-[#0B8F71]">← All locations</Link>
            <div className="mt-10 grid gap-12 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.24em] text-[#C99A2E]">NewLife Dryclean • Ranchi</p>
                <h1 className="mt-5 text-5xl font-semibold tracking-[-0.045em] text-[#172033] md:text-7xl">
                  Dry Cleaning in
                  <span className="block text-[#0B8F71]">{branch.name}.</span>
                </h1>
                <p className="mt-7 max-w-2xl text-lg leading-8 text-[#53657A]">
                  Professional garment care, steam press, finishing and convenient service from our {branch.name} location in Ranchi.
                </p>
                <div className="mt-9 flex flex-col gap-4 sm:flex-row">
                  <a href={branch.mapsUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full bg-[#0B8F71] px-7 py-4 font-semibold text-white">
                    <Navigation size={18} /> Get Directions
                  </a>
                  <Link href="/#pickup" className="inline-flex items-center justify-center gap-2 rounded-full border border-[#dbe5f4] bg-white px-7 py-4 font-semibold text-[#0B8F71]">
              ...
              <aside className="rounded-[2rem] border border-[#dbe5f4] bg-white p-8 shadow-[0_24px_60px_rgba(30,40,35,0.08)]">
                ...
                <a href={`tel:+91${DISPLAY_PHONE}`} className="mt-7 inline-flex items-center gap-2 font-semibold text-[#0B8F71]"><Phone size={17} /> {DISPLAY_PHONE}</a>
              </aside>
            </div>
          </div>
        </section>
        ...
        <section className="px-6 py-24 md:px-12">
          <div className="mx-auto max-w-7xl">
            <div className="mx-auto max-w-3xl text-center">
              <p className="text-sm font-semibold uppercase tracking-[0.24em] text-[#C99A2E]">Available Services</p>
            ...
            <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {services.map((service) => (
                <Link key={service.href} href={service.href} className="group flex gap-4 rounded-2xl border border-[#dbe5f4] bg-[#f8fbff] p-5 transition hover:border-[#9fc1e7]">
                  <CheckCircle2 size={20} className="mt-1 shrink-0 text-[#C99A2E]" />
                  <div>
                    <h3 className="font-semibold text-[#172033]">{service.name}</h3>
                    <p className="mt-1 text-sm leading-6 text-[#53657A]">{service.short}</p>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <section className="bg-[#0B8F71] px-6 py-20 text-white md:px-12">
          <div className="mx-auto grid max-w-7xl gap-8 md:grid-cols-2 md:items-center">
            ...
            <Link href="/#pickup" className="inline-flex items-center justify-center gap-2 rounded-full bg-[#f8fbff] px-8 py-4 font-semibold text-[#0B8F71] md:justify-self-end">Book Pickup <ArrowRight size={18} /></Link>
          </div>
        </section>
      </main>
      <SiteFooter />
